...
export class Engine {
    ...
    if (side === "buy") {
      fills.forEach((fill) => {
        this.balances.get(fill.otherUserId)![quoteAsset].available =
          ...
          fill.qty * parseFloat(fill.price);
  ...
  checkAndLockFunds(
    ...
    if (side === "buy") {
      const availableQuoteAsset = userBalances[quoteAsset]?.available || 0;

      if (availableQuoteAsset < Number(quantity) * Number(price)) {
        throw new Error("Insufficient funds");
      }

      userBalances[quoteAsset].available =
        availableQuoteAsset - Number(quantity) * Number(price);

      userBalances[quoteAsset].locked =
        (userBalances[quoteAsset]?.locked || 0) +
        Number(quantity) * Number(price);
    } else {
      const availableBaseAsset = userBalances[baseAsset]?.available || 0;

      if (availableBaseAsset < Number(quantity)) {
        throw new Error("Insufficient funds");
      }

      userBalances[baseAsset].available = availableBaseAsset - Number(quantity);

      userBalances[baseAsset].locked =
        (userBalances[baseAsset]?.locked || 0) + Number(quantity);
    }
  }
